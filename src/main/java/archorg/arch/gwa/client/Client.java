...
public class Client implements EntryPoint
{
  ...
  public void onModuleLoad()
  {
    ...
    URIFragmentStateManager sm = new URIFragmentStateManager(rm);
    // errors in the loading are routed using this observer
    sm.getMessage().subscribe(new Observer<String>()
    {
      @Override
      public void notice(Observable<? extends String> observable,
          String message)
      {
        rm.getMessageBW().setNevertheless(message);
      }
    });
    // TriggerBeacons can be configured to trigger a dump of the current state
    TriggerBeacon.setStateManager(sm);
